...
BinaryTree::size(const BinaryNode * subtree)
{
	if (subtree==NULL)
		...
		return(size(BinaryNode->left_) + 1 + size(BinaryNode->right_));
}
...
BinaryTree::height(const BinaryNode * subtree)
{
	...
	{
		int lDepth = maxDepth(BinaryNode->left_);
		int rDepth = maxDepth(BinaryNode->right_);
...
BinaryTree::leaves(const BinaryNode * subtree)
{
	if (subtree == NULL)
		return 0;
	if (BinaryNode->left_ == NULL && BinaryNode->right_ == NULL)
		return 1;
	else
		return getLeafCount(BinaryNode->left_) + getLeafCount(BinaryNode->right_);
